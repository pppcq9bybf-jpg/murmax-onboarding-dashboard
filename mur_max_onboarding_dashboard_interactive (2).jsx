...
import { CheckCircle2, X, Upload, ArrowLeft, ArrowRight, ShieldCheck, Sparkles, Bell } from "lucide-react";
...
const api = {
  ...
  finalize: async (role: Role, data: any) => {
    ...
    createMarketplaceProfile(role, payload);
    await wait(350);
...
// ---- Marketplace profile bootstrap ----
function createMarketplaceProfile(role: Role, data: any){
  const key = `murmax:marketplace:profiles`;
  const all = JSON.parse(localStorage.getItem(key) || '[]');
  const base = { id: `${role}-${Date.now()}`, role, createdAt: Date.now() } as any;
  if (role === 'Driver') {
    all.push({ ...base, name: data.name, equipment: data.vehicle || data.eqid || 'Unknown', hos: 'Available' });
  } else if (role === 'Dispatcher') {
    all.push({ ...base, company: data.company, drivers: (data.drivers||'').split(',').map((s:string)=>s.trim()).filter(Boolean), roleLevel: data.role||'Viewer' });
  } else if (role === 'Shipper') {
    all.push({ ...base, biz: data.biz, address: data.address, escrowLinked: !!data.pay });
  } else if (role === 'Broker') {
    all.push({ ...base, mc: data.mc, dot: data.dot, split: data.split || 'N/A' });
  }
  localStorage.setItem(key, JSON.stringify(all));
}

// ---- Reminders (COI expiry) ----
function scheduleReminder(key: string, dateISO: string){
  if (!dateISO) return;
  const existing = JSON.parse(localStorage.getItem('murmax:reminders')||'[]');
  const next = existing.filter((r:any)=> r.key !== key);
  next.push({ key, dateISO });
  localStorage.setItem('murmax:reminders', JSON.stringify(next));
}
function getDueReminders(withinDays = 14){
  const items = JSON.parse(localStorage.getItem('murmax:reminders')||'[]');
  const now = new Date();
  const due = [] as { key:string; dateISO:string }[];
  for (const r of items) {
    const d = new Date(r.dateISO);
    const diff = (d.getTime() - now.getTime()) / (1000*60*60*24);
    if (diff <= withinDays) due.push(r);
  }
  return due;
}
...
// ---- Role Configs with RBAC and COI expiry ----
const driverSteps = (state: any, set: (p: any) => void): Step[] => [
  ...
  { title: "Documents", valid: (s) => !!s.cdlFile && !!s.coiFile && !!s.coiExp,
    Content: () => (
      <Row>
        <Field label="Upload CDL (PDF/Image)"><UploadField value={state.cdlFile} onChange={(f)=>set({ cdlFile:f })} accept=".pdf,.png,.jpg,.jpeg"/></Field>
        <Field label="Upload Insurance COI (PDF)"><UploadField value={state.coiFile} onChange={(f)=>set({ coiFile:f })} accept=".pdf"/></Field>
        <Field label="COI Expiry Date">
          <Input type="date" value={state.coiExp||""} onChange={e=>{ set({ coiExp:e.target.value }); scheduleReminder('driver:coi:'+ (state.cdl||'unknown'), e.target.value); }} className="bg-neutral-900 border-neutral-700"/>
        </Field>
        <Field label="Vehicle Registration (PDF)"><UploadField value={state.regFile} onChange={(f)=>set({ regFile:f })} accept=".pdf"/></Field>
      </Row>
    ) },
...
const dispatcherSteps = (state:any,set:(p:any)=>void): Step[] => [
  ...
  { title: "COI & Training", valid: (s)=>!!s.trainingDone && !!s.coiExp,
    Content: ()=> (
      <Row>
        <Field label="Upload COI (PDF)"><UploadField value={state.coi} onChange={(f)=>set({coi:f})} accept=".pdf"/></Field>
        <Field label="COI Expiry Date"><Input type="date" value={state.coiExp||""} onChange={e=>{ set({ coiExp:e.target.value }); scheduleReminder('dispatcher:coi:'+ (state.company||'unknown'), e.target.value); }} className="bg-neutral-900 border-neutral-700"/></Field>
        <div className="md:col-span-2 space-y-2">
          ...
          <label className="text-sm flex items-center gap-2"><input type="checkbox" checked={!!state.trainingDone} onChange={e=>set({trainingDone:e.target.checked})}/> I completed both modules</label>
        </div>
      </Row>
    ) },
];
  ...
  { title: "Registration", valid: (s)=>!!s.mc || !!s.dot,
    Content: ()=> (
      <Row>
        ...
        <Field label="Broker Authority (PDF)"><UploadField value={state.auth} onChange={(f)=>set({auth:f})} accept=".pdf"/></Field>
        <Field label="COI (PDF)"><UploadField value={state.coi} onChange={(f)=>set({coi:f})} accept=".pdf"/></Field>
      </Row>
    ) },
  { title: "Terms, Escrow & COI", valid: (s)=>!!s.terms && !!s.split && !!s.coiExp,
    Content: ()=> (
      <Row>
        ...
        <Field label="COI Expiry Date"><Input type="date" value={state.coiExp||""} onChange={e=>{ set({ coiExp:e.target.value }); scheduleReminder('broker:coi:'+ (state.mc||state.dot||'unknown'), e.target.value); }} className="bg-neutral-900 border-neutral-700"/></Field>
      </Row>
    ) },
];
...
export default function MurMaxOnboardingDashboard(){
  ...
  // Reminder scanner
  useEffect(()=>{
    const due = getDueReminders(14);
    if (due.length){
      for (const r of due){
        pushToast(`Reminder: ${r.key} expiring by ${new Date(r.dateISO).toLocaleDateString()}`);
      }
    }
  },[]);
  ...
  const onFinish = async () => {
    ...
    if (res.ok) {
      setBanner(`${role} onboarding complete — profile created in Marketplace (#${res.id}). 🎉`);
  ...
  const onOpenMarketplace = () => {
    ...
    // If running alongside Marketplace canvas, this hash can be a convention to switch views.
...
function DevSelfTests(){
  ...
  // Test 3: dispatcher RBAC must be selected & COI expiry required
  const dummyDisp:any = { company:"X", ein:"Y", role: undefined, coiExp: "" };
  const ok3 = dispatcherSteps(dummyDisp, ()=>{} )[1].valid(dummyDisp) === false && dispatcherSteps(dummyDisp, ()=>{} )[2].valid(dummyDisp) === false;

  // Test 4: COI reminders schedule
  scheduleReminder('test:coi','2030-01-01');
  const ok4 = getDueReminders(365*10).some(r=> r.key==='test:coi');
  ...
  return (
    <div className="fixed right-3 bottom-3 text-[10px] text-neutral-300 bg-neutral-900/80 border border-neutral-700 rounded-lg px-2 py-1">
      Tests: DriverReg={pass(ok1)} • ShipperPay={pass(ok2)} • RBAC+COI={pass(ok3)} • ReminderSched={pass(ok4)}
    </div>
  );
}
