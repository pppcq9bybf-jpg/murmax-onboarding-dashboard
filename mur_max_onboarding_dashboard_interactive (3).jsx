import { Home, Truck, DollarSign, User, MessageSquare, FileText, Settings, HelpCircle, Fuel, BarChart3, ClipboardList, Gift, MapPin, Bell, Filter, Users, ListChecks, SendHorizontal, X, Mail, FileDown, CheckCircle2, Loader2, UserCircle2 } from "lucide-react";
...
export default function MurMaxTruckingAppUI() {
  ...
  // HYBRID LOGIC: A (In-House First) then B (Lowest RPM Under Budget)
  ...
  // Mock availability + partner network for Instant logic
  const inHouseOffers = [ { carrier: "MurMax Express®", rpm: 2.35, eta: 3.2, equipment: "26' Box", onTime: 0.98 } ];
  const partnerOffers = [
    { carrier: "Queen City Freight", rpm: 2.20, eta: 4.0, equipment: "26' Box", onTime: 0.96 },
    { carrier: "TX Hub Carriers", rpm: 2.10, eta: 5.0, equipment: "Reefer", onTime: 0.95 },
    { carrier: "Seaport Freight", rpm: 2.28, eta: 3.8, equipment: "53' Van", onTime: 0.94 },
  ];
  ...
  const runNoStringsInstant = () => {
    ...
    if (!shipperForm.origin || !shipperForm.destination || !shipperForm.budgetUSD || !shipperForm.equipment) {
      log("Missing required fields (origin/destination/budget/equipment).");
      pushNotice("Enter origin, destination, budget, equipment.");
      return;
    }
    // A: In-House First
    ...
    if (ih) {
      if (ih.rpm * 300 <= shipperForm.budgetUSD) { // demo 300mi
        log("In-house within budget → Booking MurMax Express®.");
        pushNotice("Instant Booked: MurMax Express® (in-house)");
        setRateConData({ loadId: "AUTO-INSTANT", shipper: "Shipper, Inc.", carrier: "MurMax Express®", broker: "Direct", origin: shipperForm.origin, destination: shipperForm.destination, equipment: shipperForm.equipment, rateUSD: Math.round(ih.rpm * 300), terms: defaultTerms });
        setRateConOpen(true);
        log("Escrow hold initialized.");
        return;
      } else {
        log("In-house over budget → Falling back to partner network.");
      }
    } else { log("No in-house equipment match → Partner search."); }

    // B: Lowest RPM Under Budget
      ...
      <main className="p-4">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
          <CardShell>
            ...
            {profiles.length === 0 ? (
              <p className="text-sm text-neutral-400">No profiles yet. Complete onboarding to auto-create profiles.</p>
            ) : (
              <ul className="space-y-2 max-h-56 overflow-auto text-sm">
                {profiles.map((p, i) => (
                  <li key={i} className="flex items-center justify-between bg-neutral-800 border border-neutral-700 rounded-xl px-3 py-2">
                    <span className="truncate">{p.role}: {p.name || p.company || p.biz || p.id}</span>
          ...
          {/* Driver/Dispatcher/Shipper panels from before */}
...
function FleetMapCard() {
  ...
  return (
    <CardShell>
      ...
      <div className="h-64 rounded-xl bg-[linear-gradient(135deg,#111,transparent)] border border-neutral-800 flex items-center justify-center mb-3">
        <div className="text-center text-neutral-400">
          <MapPin className="w-8 h-8 mx-auto mb-2" />
          <p>Map placeholder — integrate Mapbox/Google Maps; pins show trucks / loads / POIs.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Section title="Quotes & Bids (Live)">
          <ul className="space-y-2 text-sm">
            {[{ carrier: "MurMax Express®", price: 1180, eta: "Pick 15:00", equip: "26' Box" }, { carrier: "Queen City Freight", price: 1300, eta: "Pick 17:00", equip: "26' Box" }, { carrier: "TX Hub Carriers", price: 1140, eta: "Pick 19:00", equip: "Reefer" }].map((q, i) => (
              <li key={i} className="flex items-center justify-between bg-neutral-800 border border-neutral-700 rounded-xl px-3 py-2">
                <div>
...
// =====================
function DevTests() {
  // Test 1: HOS-aware dispatch picks an Available driver
  ...
  // Test 3: Hybrid Instant booking rule — prefers in-house if within budget
  const budget = 800; // 300mi x RPM threshold
  const inHouse = [{ rpm: 2.5 }];
  const partners = [{ rpm: 2.2 }, { rpm: 2.8 }];
  const chosen = (inHouse[0].rpm * 300 <= budget) ? 'INHOUSE' : (partners.sort((a,b)=>a.rpm-b.rpm)[0].rpm * 300 <= budget ? 'PARTNER' : 'NONE');
  const pass3 = chosen === 'INHOUSE' ? "PASS" : "FAIL";

  // Test 4: Profiles list renders
  const profiles = JSON.parse(localStorage.getItem('murmax:marketplace:profiles')||'[]');
  const pass4 = Array.isArray(profiles) ? "PASS" : "FAIL";
  ...
  return (
    <div className="fixed right-3 bottom-3 text-[10px] text-neutral-300 bg-neutral-900/80 border border-neutral-700 rounded-lg px-2 py-1">
      DevTests: HOS={test1} • RateCon={test2} • Instant={pass3} • Profiles={pass4}
    </div>
  );
}
