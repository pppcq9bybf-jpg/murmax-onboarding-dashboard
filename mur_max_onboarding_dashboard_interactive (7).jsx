...
// === NEW: simple alias for onboarding apps ===
;(window as any).murmaxOnboardingFinalize = (role: string, payload: any) => {
  return (window as any).murmaxHandoff(role, payload);
};
// === NEW: postMessage listener for cross-window handoff ===
window.addEventListener('message', (e: any) => {
  try {
    if (e?.data?.type === 'MURMAX_ONBOARDING_FINALIZED') {
      (window as any).murmaxOnboardingFinalize(e.data.role, e.data.payload || {});
    }
  } catch {}
});
