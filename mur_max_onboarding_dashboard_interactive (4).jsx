import { Home, Truck, DollarSign, User, MessageSquare, FileText, Settings, HelpCircle, Fuel, BarChart3, ClipboardList, Gift, MapPin, Bell, Filter, Users, ListChecks, SendHorizontal, X, Mail, FileDown, CheckCircle2, Loader2, UserCircle2, Plus } from "lucide-react";
...
export default function MurMaxTruckingAppUI() {
  ...
  const refreshProfiles = () => {
    try {
      ...
      setProfiles(list);
      // Determine highlight target: prefer lastCreatedId, else newest when count increased
  ...
  useEffect(() => {
    ...
    if (el) {
      ...
      // Remove stored lastCreatedId so we don't keep highlighting across sessions
...
function DevTests() {
  ...
  // Test 3: Profiles filtering and highlight id logic
  const key = 'murmax:marketplace:profiles';
  const list = JSON.parse(localStorage.getItem(key) || '[]');
